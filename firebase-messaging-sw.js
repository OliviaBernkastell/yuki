...
async function cleanupOldCaches() {
  ...
  const oldCacheNames = cacheNames.filter(
    (cacheName) =>
      (cacheName.startsWith("yuki-pwa-") ||
        cacheName.startsWith("yuki-data-")) &&
...
self.addEventListener("notificationclick", (event) => {
  ...
  switch (action) {
    case "view_tasks":
      targetUrl = "/?action=view-tasks";
      break;
    case "add_task":
      targetUrl = "/?action=add-task";
      break;
    case "dismiss":
...
async function clearAllCaches() {
  const cacheNames = await caches.keys();
  const yukiCaches = cacheNames.filter(
    (name) => name.startsWith("yuki-pwa-") || name.startsWith("yuki-data-")
  );

  return Promise.all(yukiCaches.map((cacheName) => caches.delete(cacheName)));
}
